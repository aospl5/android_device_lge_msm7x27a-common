...
public class LgeMsm7x27aRIL extends RIL implements CommandsInterface {
    private boolean sentHwBootstrap = false;

    public LgeMsm7x27aRIL(Context context, int networkMode, int cdmaSubscription) {
    ...
    lgeSendCommand(int command) {
        ...
        send(rrLSC);
        riljLog("LgeMsm7x27aRIL: LGE COMMAND " + (command) + " sendt");
    }
    ...
    private void restartRild() {
        if (RILJ_LOGD) {
            riljLog("LgeMsm7x27aRIL: restartRild started");
        }
